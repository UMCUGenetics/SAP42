...
function selectExperiments($galaxy)
{
	...
	$file = fopen("dataXSQ.txt", "r") or exit("Unable to open file!");
	//Output a line of the file until the end is reached

	$content .= "<form action=\"\" method=\"POST\">\n";
	$content .= "<input type=\"hidden\" name=\"page\" value=\"preMapping\" />\n";
	$content .= "<input type=\"hidden\" name=\"solid_type\" value=\"xsq\">\n";
	if (strcmp($galaxy,'') != 0){
...
// 					$tmp_content .= "<input type=\"submit\"><input type=\"reset\">";
	...
	$file = fopen("dataXSQ.tmp.txt", "r") or exit("Unable to open file!");
...
// 	$content = $aData;
	...
	$file = fopen("data5500.tmp.txt", "r") or exit("Unable to open file!");
	while(!feof($file))
	{
		$line = fgets($file);
	...
	foreach($aData as $value) {
		...
		if (preg_match("/\d{8}/", $csfasta[$value][0], $matches)) {
...
// 	echo $aData;
	...
	$content .= "<input type=\"hidden\" name=\"platform\" value=\"5500\">";
	
	$content .= "<h3>Path to working directory on " . $configuration['SUBMITNODE'] . ":</h3>\n";
	$content .= "<input type=\"text\" name=\"pwd\" size=50 value=\"/hpc/cog_bioinf/data/mapping/diagnostiek\">";
...
// 		$content .= "<tr><td>" . $csfasta[$value][0] . "</td><td>" . $csfasta[$value][1] . "</td></tr>\n";
// 		$content .= "<tr><td>Project name:</td><td>" . $selected_name . "</td></tr>\n";
// 		$content .= "<tr><td>Reference: </td><td>" . $ref[$selected_reference] . "</td></tr>\n";

// 		$content .= "<tr><td>Alignemnt tool:</td><td>" . $alignmentTool[$selected_tool] . "</td></tr>\n";
// 		$content .= "<tr><td>Mapping settings:</td><td>" . $mapSettings[$selected_settings] . "</td></tr>\n";
...
// 		fwrite($config, "CSFASTA\t". $csfasta[$value][3] . "\n");
		fwrite($config, "XSQ\t". $csfasta[$value][3] . "\n");
// 		$wd = substr($csfasta[$value][3], 0, -4);
		$wd = basename($csfasta[$value][3], ".xsq");
// 		fwrite($config, "QUAL\t$quality\n");
// 		fwrite($config, "PWD\t$pwd/" . $wd . "\n");
...
// 	fclose($fp);
	...
	if ($configuration['RELEASE'] === 'stable'){
		...
		$content .= "<h4>SAP42 create -vE -X $code</h4>";
	}else{
		$content .= "Execute the following command on " . $configuration['SUBMITNODE'] . ":";
		$code = str_replace("/tmp/", "", $tmpFile);
		$content .= "<h4>" . $configuration['SCRIPTROOT'] . "/SAP42_create -vE -X $code</h4>";
	}
